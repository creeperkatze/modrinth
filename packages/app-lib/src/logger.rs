/*
    ...
    The default is refract_lib=info, meaning only logs from refract_lib will be displayed, and at the info or higher level.
...
pub fn start_logger(_app_identifier: &str) -> Option<()> {
    ...
    let filter = tracing_subscriber::EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| {
            tracing_subscriber::EnvFilter::new("refract_lib=info,refract=info")
        });
...
pub fn start_logger(app_identifier: &str) -> Option<()> {
    ...
    let filter = tracing_subscriber::EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| {
            tracing_subscriber::EnvFilter::new("refract_lib=info")
        });
