...
defineExpose({
	async show(errorVal, context, canClose = true, source = null) {
		...
		if (errorVal.message && errorVal.message.includes('Minecraft authentication error:')) {
			...
			title.value = 'Error initializing Refract'
			...
			<div class="markdown-body">
				<template v-if="errorType === 'minecraft_auth'">
					<template v-if="metadata.network">
						...
						<p>
							It looks like there were issues with Refract connecting to Microsoft's
					...
					<template v-else-if="metadata.hostsFile">
						<h3>Network issues</h3>
						<p>
							Refract tried to connect to Microsoft / Xbox / Minecraft services, but the
				...
				<template v-if="errorType === 'directory_move'">
					<template v-if="metadata.readOnly">
						...
						<p>
							It looks like Refract is unable to write to the directory you selected.
					...
					<template v-else>
						<p>
							Refract is unable to migrate to the new directory you selected. Please
				...
				<template v-else-if="errorType === 'state_init'">
					<p>
						Refract failed to load correctly. This may be because of a corrupted file, or
				...
				<template v-else-if="errorType === 'no_loader_version'">
					<p>Refract failed to find the loader version for this instance.</p>
